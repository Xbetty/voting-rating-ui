    ...
    <el-card class="box-card">
      ...
      <el-row type="flex" justify="start">
        ...
        <el-form :inline="true" v-model="searchContent">
          ...
          <el-form-item>
            <el-input v-model="searchContent.keywords" placeholder="请输入项目名称或讲师姓名" style="margin-right:10px;width:200px;" clearable></el-input>
          </el-form-item>
      ...
      <div>
        <el-table
          ...
          <el-table-column
            ...
            label="分享时间">
            <template slot-scope="scope">
              <span>{{formatTime(scope.row.shareTime)}}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="projectStatus"
          ...
          <el-table-column
            ...
            label="平均得分">
            <template slot-scope="scope">
              <span>{{scope.row.scoreResult.toFixed(2)}}</span>
            </template>
          </el-table-column>
        </el-table>
        <!-- 分页 -->
...
export default {
  ...
  methods: {
    ...
    toProjectDetails (_id) {
      this.$router.push({name: 'scoreDetails', query: {_id: _id, projectType: 1}})
    },
    // 搜索操作
    handleSearch () {
