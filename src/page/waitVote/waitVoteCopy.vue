      ...
      <div class="project_content">
        ...
        <el-row v-else v-for="(item,key) in projectContent" :key="key" class="project_content_one">
            <div class="project_detail">
              ...
              <div class="project_detail_name">发起人 ： {{item.publisher}}</div>
              <div class="project_detail_time">发起时间 ： {{formatTime(item.createAt)}}</div>
              ...
              <el-button type="primary" class="score_button" @click="operatePro(item)">立即投票</el-button>
            </div>
          </el-row>
        </div>
      <div>
    ...
    getProjectList (page) {
      projectAPI.projectList(this.searchContent).then((res) => {
        ...
        if (res.code === '0000') {
          ...
          this.$store.commit('getWaitVoteCount', this.total)
        }
      })
      projectAPI.waitOperateCounts({votingType: 2}).then((res) => {
        if (res.code === '0000') {
          this.$store.commit('getWaitVoteCount', res.data.total)
        }
      })
    },
    // 立即投票
